...
// Désactive la gestion des paiements directement dans les consultations.
// Les factures et paiements seront gérés séparément dans Dolibarr.
$disableConsultationPayment = getDolGlobalInt(
    'CABINETMED_DISABLE_CONSULTATION_PAYMENT'
);
...
$arrayfields=array(
	...
	//'typepayment'=>array('label'=>"TypePaiement", 'checked'=>1, 'enabled'=>1),
        'amountpayment'=>array('label'=>"MontantPaiement", 'checked'=>0, 'enabled'=>!$disableConsultationPayment),
        'typepayment'=>array('label'=>"TypePaiement", 'checked'=>0, 'enabled'=>!$disableConsultationPayment),
);
// Extra fields
...
// Lorsqu'on désactive les paiements de consultation,
// on force tous les montants à zéro.
//
// DoliMed exige normalement qu'au moins un des champs soit renseigné.
// Le champ montant_tiers à 0 permet de satisfaire cette validation
// sans créer de paiement ou de facture.
if (
    $disableConsultationPayment
    && in_array($action, array('add', 'update'), true)
) {
    $_POST['montant_cheque'] = '';
    $_POST['montant_carte'] = '';
    $_POST['montant_espece'] = '';
    $_POST['montant_tiers'] = '0';
...
if (! ($socid > 0)) {
	...
	if ($action == 'create' || $action == 'edit') {
		...
		print '<input type="radio" class="flat" name="typevisit" value="CS" id="cs"'.($object->typevisit=='CS'?' checked="checked"':'').'> <label for="cs"></label>';
		//print ' &nbsp; &nbsp; ';
